...
public class Monopoly {
    ...
    static int d1=0, d2=0; //Dices (1-4) * Not (1-6) because of small map and big jail out chance
    static int cntPair = 0; //count series of pairs for current player
    ...
    public static void main(String[] args) {
        ...
        while (!endGame)
        {
            String action1 = "("+pink+"R"+normal+")oll dices; ";
            String action2 = "("+pink+"U"+normal+")pgrade something; ";
            String action3 = "("+pink+"S"+normal+")ell something ";
            String action3a =  "("+pink+"P"+normal+")ass! (Prevention from next pair and go on Jail) ";

            String action4 = "Beg for ("+pink+"M"+normal+")ercy; "; //in Jail
            String action5 = "("+pink+"P"+normal+")ay Tax 200 and roll dices; "; //in Jail


            String txtChooseAction = "";

            boolean isPlayerFree = true;  //Just card is not enough, we must be shure that other KEY OPTIONS dont work in Jail

            //Pre Action - If player is in JAIL
            if (plWhere[curPlayer]==18) {
                if (!hadPrisonCard[curPlayer]) {
                    txtChooseAction += "Играч " + plName[curPlayer] + " изберете действие: " + action4 + action5;
                    isPlayerFree=false;
                }
                else  System.out.println("Използвахте картата си за излизане от затвора"); hadPrisonCard[curPlayer] = false;

                //Action from JAIL
                System.out.print(txtChooseAction); //choose option
                String act = scn.nextLine();

                //Paid Tax
                if (!isPlayerFree && act.equalsIgnoreCase("P"))
                {
                    Institutions.simplePayToInstitution(curPlayer, 200);
                    System.out.println("Гаранцията ви е платена и вие сте освободен.");
                    System.out.println(); isPlayerFree = true;
                }

                //Wait for pair
                if (!isPlayerFree && act.equalsIgnoreCase("M"))
                {
                    d1 = rollDice(); d2 = rollDice();
                    if (d1==d2) {       //succes
                        cntPair = 0;    //new beginning
                        System.out.printf("Молбите ви докоснаха сърцата на управниците и вие бяхте освободен!", d1, d2);
                        System.out.println(); isPlayerFree = true;
                    }
                    else  System.out.printf("Никой не ви обърна внимание. Може би трябва да развържете кесията си следващия път ;)", d1, d2);
                    System.out.println();
                }
            }

            //Main Action (When player is Free)
            if (isPlayerFree)
            {
                txtChooseAction = "Играч " + plName[curPlayer] + " изберете действие: " + action1+action2+action3;
                if (cntPair==1) txtChooseAction += action3a;
            ...
            if (d1!=d2) {curPlayer++; if (curPlayer==4) curPlayer=1;} //When pair happens Go Again = More FUN

            //After all actions is completed let show new MAP
            ProPrintSettings.showMAP();

        }
    }

    public static void choosedActionRollDices()
    {
        //Roll dices
            d1 = rollDice(); d2 = rollDice();
            if (d1==d2) cntPair++;                          //Count series of pairs
            else cntPair=0;                                 //When 1 pair happens > Go Again!
            ...
            if (cntPair==2) {
                ...
                if (hadPrisonCard[curPlayer]) {
                    ...
                    System.out.println("Отнета ви е и картата за излизане от затвора");
                }
                Institutions.goJail(curPlayer); //When 2 pairs happens > Go Jail!
            }
    ...
    public static void checkActionsOnNewField()  //Full Action
    {
        ...
        //Jump Jail   --- must be before showMAP --
        if (newField == 18) newField=19; //let go around jail

        //When cross start field *** MUST be before update plWhere position 6-10 rows below
        ...
        plWhere[curPlayer] = newField; movePlayersToFields(); //jail if (plWhere[curPlayer]!=18)
        ...
        //Universitat field
        if (newField ==3 ) Institutions.goUniversitat(curPlayer);
    ...
    public static boolean[] hadUniversitatyCard = new boolean[4]; //no one in beginning
